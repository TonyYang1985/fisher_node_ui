...
const Uploador = React.forwardRef<FileUploadProps, any>((props, ref) => {
  ...
  const uploadedFileCount = React.useRef<number>(0);
  const hasFiles = ObjectUtils.isNotEmpty(filesState);
  const disabled = props.disabled || uploadingState;
  const chooseButtonLabel = props.chooseLabel || props.chooseOptions?.label || localeOption('choose', 'en');
  const uploadButtonLabel = props.uploadLabel || props.uploadOptions?.label || localeOption('upload', 'en');
  const cancelButtonLabel = props.cancelLabel || props.cancelOptions?.label || localeOption('cancel', 'en');
  ...
  const onFileSelect = (event) => {
    ...
    if (props.mode === 'basic' && currentFiles.length > 0) {
      (fileInputRef.current as any).style.display = 'none';
    }
  };
  ...
  const validate = (file) => {
    if (props.maxFileSize && file.size > props.maxFileSize) {
      ...
      if (props.mode === 'advanced') {
        (messagesRef.current as any).show(message);
      }
  ...
  const onDragOver = (event) => {
    if (!disabled) {
      event.dataTransfer.dropEffect = 'copy';
      DomHandler.addClass(contentRef.current as any, 'p-fileupload-highlight');
      event.stopPropagation();
      event.preventDefault();
    }
  };
  ...
  const onDragLeave = (event) => {
    if (!disabled) {
      event.dataTransfer.dropEffect = 'copy';
      DomHandler.removeClass(contentRef.current as any, 'p-fileupload-highlight');
    }
  };
  ...
  const onSimpleUploaderClick = () => {
    !disabled && hasFiles ? upload() : (fileInputRef.current as any).click();
  };
  ...
  const createAdvanced = () => {
    const otherProps = ObjectUtils.findDiffKeys(props, Uploador.defaultProps);
  ...
  const createBasic = () => {
    ...
    const buttonClassName = classNames(
      ...
      { 'p-fileupload-choose-selected': hasFiles, 'p-disabled': disabled, 'p-focus': focusedState },
...
Uploador.displayName = 'FileUpload';
Uploador.defaultProps = {
  __TYPE: 'FileUpload',
  id: null,
  name: null,
  url: null,
...
export const FileUpload = React.memo(Uploador);
